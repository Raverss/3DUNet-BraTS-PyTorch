...
def parse_seg_args():
    ...
    parser.add_argument(
        '--optim', type=str, default='adamw', help='optimizer', choices=['adam', 'adamw', 'sgd']
    )
    parser.add_argument(
        '--beta1', default=0.9, type=float, metavar='M', help='momentum for sgd, beta1 for adam'
    )
    ...
    parser.add_argument(
        '--milestones', type=int, nargs='+', default=[60, 80], help='milestones for multistep decay'
    )
    parser.add_argument(
        '--lr_gamma', type=float, default=0.1, help='decay factor for multistep decay'
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default='unet',
        choices=['unet', 'multiencoder_unet', 'unetr'],
        help='Architecuture of the U-Net',
    )
    parser.add_argument(
        '--block',
        type=str,
    ...
    parser.add_argument(
        '--deep_supervision', action='store_true', help='whether use deep supervision'
    )
    parser.add_argument(
        '--ds_layer', type=int, default=4, help='last n layer to use deep supervision'
    )

    # eval
    parser.add_argument(
        '--save_model', action='store_true', default=False, help='whether save model state'
    )
    parser.add_argument(
        '--save_pred', action='store_true', default=False, help='whether save individual prediction'
    )
    ...
    parser.add_argument(
        '--patch_overlap', type=float, default=0.5, help='overlap ratio between patches'
    )
